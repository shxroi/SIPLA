...
const BookingRegular = () => {
  ...
  const fetchBookings = async () => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:3000/api/booking/admin/regular?${queryParams}`, {
        headers: {
      ...
      if (error.response?.status === 401) {
        ...
        setError('Gagal memuat data booking regular');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Manajemen Booking Regular</h1>
      </div>
        ...
        <>
          {bookings.length > 0 ? (
            ...
            <div className="text-center py-8 bg-gray-50 rounded-md">
              Tidak ada data booking regular.
            </div>
          )}
